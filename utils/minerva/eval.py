...
class MinervaEvaluator(BaseMathEvaluator):
    """Minerva: question/answer jsonl, \\boxed extraction, grade_answer for LaTeX equivalence."""
    ...
    def build_prompt(self, tokenizer, question: str, *, enable_thinking: bool = False) -> str:
        messages = [
            {"role": "system", "content": MINERVA_SYSTEM_PROMPT},
...
def evaluate_minerva_accuracy(
    ...
    """Convenience entry point: run Minerva evaluation via the base evaluator."""
    return MinervaEvaluator().evaluate(
        model=model,
        tokenizer=tokenizer,
        test_path=test_path,
        device=device,
